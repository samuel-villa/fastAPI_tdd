...
def test_model_structure_foreign_key(db_inspector):
    ...
    produtype_product_foreign_key = next(
        (
            fk
    ...
    assert produtype_product_foreign_key is not None
...
#     assert columns["product_product_type"]["type"].length == 100
...
def test_model_structure_unique_constraints(db_inspector):
    ...
    assert any(
        constraint["name"] == "uq_prod_prodtype_prod_id_prod_type_id"
        for constraint in constraints
    )
