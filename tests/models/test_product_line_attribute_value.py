...
def test_model_structure_foreign_key(db_inspector):
    ...
    attribute_value_foreign_key = next(
        (
            ...
            if set(fk["constrained_columns"]) == {"attribute_value_id"}
            or set(fk["constrained_columns"]) == {"product_line_id"}
        ),
        None,
    )
...
#     assert columns["product_line_attribute_value"]["type"].length == 100
...
def test_model_structure_unique_constraints(db_inspector):
    ...
    assert any(
        constraint["name"] == "uq_attrval_prodline_attribute_value_id_produ_line_id"
        for constraint in constraints
    )
